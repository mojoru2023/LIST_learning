...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into ctolib_cpp_links_details (link,title,DE) values (%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError:
...
#     # 使用cursor()方法获取操作游标
    ...
    cursor = connection.cursor()
    for i in range(1,39):
        sql = 'select * from ctolib_cpp_links where id = %s ' % i
        # #执行sql语句
        cursor.execute(sql)
        # #获取所有记录列表
...
#
# create table ctolib_cpp_links_details(
