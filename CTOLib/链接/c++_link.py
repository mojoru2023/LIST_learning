...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into ctolib_cpp_links (title,DE,link) values (%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError:
...
if __name__ == '__main__':
    url = 'https://www.ctolib.com/cpp/categoriesallsub.html'
