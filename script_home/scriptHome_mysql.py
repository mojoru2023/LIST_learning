...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into scriptHome_mysql (title,link) values (%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
...
if __name__ == '__main__':
    for offset in range(1,112):
        url = 'https://www.jb51.net/list/list_112_' + str(offset) + '.htm'
