...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into scriptHome_mongodb (title,link) values (%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
...
if __name__ == '__main__':
    for offset in range(1,9):
        url = 'https://www.jb51.net/list/list_239_' + str(offset) + '.htm'
...
#
# create table scriptHome_mongodb(
