...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into ajax_highcharts_BD (link) values (%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
...
if __name__ == '__main__':
    for item in range(0,1040,10):
        url = 'https://www.baidu.com/s?wd=ajax%20highcharts&pn=' + str(item) + '&oq=ajax%20highcharts&ie=utf-8&rsv_idx=1&rsv_pq=8749835400034570&rsv_t=581exa%2BhnFqtmF%2BrwvaS9H%2B84sWu7w9MVdVp8AkcFtXZywBTvPPAJ%2BGDt9c&rsv_page=-1'
...
# create table ajax_highcharts_BD(
# id int not null primary key auto_increment,
# link text
# ) engine=InnoDB  charset=utf8;

# drop table ajax_highcharts_BD;
